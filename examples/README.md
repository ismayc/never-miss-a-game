...
|---|---|---|---|---|
| 01 | `01-preferences.json` | main | Input. Eleven teams, keyed by sport and abbreviation, each with a `why` note. A snapshot of `../preferences.json`. | The `why` lines. They are the raw material for the digest's "why I care" clauses, and the policy tells the agent to use them without repeating them. Compare them with the lines in 06. |
