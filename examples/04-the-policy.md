...
| The output contract | One message, not a report. One framing sentence, then the games by day, one line each: who, when, why I care (nine words, specific, different every line). One clock throughout. A hard cap, and a cut order for when it is exceeded. If nothing is worth watching, say so in two sentences and stop. A tone dial (`snark`, 1 to 5, from `preferences.json`) that changes the reasons and never the facts. |
